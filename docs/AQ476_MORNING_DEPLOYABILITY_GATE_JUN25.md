# AQ-476 morning deployability gate, Jun 25

Gate run from `/root/baycast-dev` after the required fast-forward check against `origin/main`.

Head at gate start: `64e18419d42f67d5cfdd3124ef6d8cd28658c095`.
Node used: `v22.22.2`.
npm used: `10.9.7`.

Verdict: PASS. The repo is deployable. All requested gates passed on Node 22, so no Node 20 retry was needed. I did not change application code.
...
## Baseline and diff check

Command:

```bash
node -v && npm -v && git rev-parse HEAD && git status --short && git diff --check
```
...
v22.22.2
10.9.7
64e18419d42f67d5cfdd3124ef6d8cd28658c095
```

`git status --short` and `git diff --check` had no output.
...
npm also printed an update notice for npm 11.17.0. It is informational only.
...
> vitest run
 ...
 ✓ __tests__/agent-forecast-route.test.ts (1 test) 64ms
 ✓ __tests__/utils.test.ts (37 tests) 57ms
 ...
 Test Files  12 passed (12)
   ...
   Start at  07:01:49
   Duration  12.38s (transform 274ms, setup 658ms, import 299ms, tests 267ms, environment 9.40s)
```
...
The public checks reached `https://baycast-p.vercel.app` successfully. The build printed the known webpack cache warning about a large serialized string. Nothing in the gate points to a code bug.
