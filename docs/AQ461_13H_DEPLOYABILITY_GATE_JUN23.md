# AQ-461 13h deployability gate, Jun 23

Run from `/root/baycast-dev` on `main`.

Baseline after fast-forward: `84ec7a77a55a3b366f8b2a145bff13aaed399cf1`.

Environment:

```text
UTC run: 2026-06-23 13h
Node: v22.22.2
npm: 10.9.7
```
...
The working tree was clean before the run. The clone was already up to date with `origin/main`. All required gates passed on Node 22, so no Node 20 retry was needed. No product copy or runtime code was patched.
...
```text
$ git status --short && git branch --show-current && git fetch origin && git pull --ff-only origin main && git rev-parse HEAD && node -v && npm -v
main
From https://github.com/Smildous/baycast
 * branch            main       -> FETCH_HEAD
Already up to date.
84ec7a77a55a3b366f8b2a145bff13aaed399cf1
v22.22.2
10.9.7
```
...
> vitest run
 ...
 ✓ __tests__/supabase-admin.test.ts (13 tests) 97ms
 ✓ __tests__/agent-forecast-route.test.ts (1 test) 79ms
 ✓ __tests__/utils.test.ts (37 tests) 55ms
 ...
 ✓ __tests__/news-context.test.ts (5 tests) 9ms
 ...
 Test Files  12 passed (12)
   ...
   Start at  13:02:47
   Duration  13.17s (transform 284ms, setup 771ms, import 284ms, tests 293ms, environment 9.99s)
```
...
The helper inspection matched the recent deployability gate pattern: the required scripts are the current package scripts, the latest comparable gate was AQ-458, and the 110-test Vitest count still matches this run.
